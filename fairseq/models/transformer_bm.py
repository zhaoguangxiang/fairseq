...
class TransformerBMEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args):
        ...
        if self.qk_bagging:
            self.self_attn = MultiheadAttentionEncoderBagging(self.embed_dim, args.encoder_attention_heads, args=args,)
        else:
...
def linear_bm(args, in_dim,out_dim,cur_fc='in'):
    ...
    bm_in_a = args.bm_in_a if 'bm_in_a' in args else 5  # 6 / sqrt(a +1)  a=1 就是根号3倍 a= 0 就是根号6倍， a=11就是根号1/2倍
    bm_out_a = args.bm_out_a if 'bm_out_a' in args else 5  # 6 / sqrt(a +1)  a=1 就是根号3倍 a= 0 就是根号6倍， a=11就是根号1/2倍
...
class TransformerBMDecoder(FairseqIncrementalDecoder):
    ...
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, final_norm=True):
        ...
        self.layers = nn.ModuleList([])
        if args.qk_bagging > 0:
            self.layers.extend([
                TransformerBMDecoderLayerBagging(args=args, no_encoder_attn=no_encoder_attn)
                for i in range(args.decoder_layers)
            ])
        else:
            self.layers.extend([
                    TransformerBMDecoderLayer(args=args, no_encoder_attn=no_encoder_attn)
                    for i in range(args.decoder_layers)
                ])
...
class MultiheadAttentionEncoderBagging(nn.Module):
    """Multi-headed attention.

    See "Attention Is All You Need" for more details.
    """
    def __init__(self, embed_dim, num_heads, args,):
        super().__init__()
        # for qk bagging
        self.qk_bagging = args.qk_bagging if 'qk_bagging' in args else 0  # 0 nothing 1 max 2 mean
        self.qk_big = args.qk_big if 'qk_big' in args else 1
        self.bagging_num = args.bagging_num if 'bagging_num' in args else 1
        self.qk_dropout = args.qk_dropout if 'qk_dropout' in args else 0
        if self.qk_dropout:
            self.qk_Dropout = nn.Dropout(self.qk_dropout)
        else:
            self.qk_Dropout = None
        self.embed_dim = embed_dim
        self.num_heads = num_heads
        self.dropout = args.attention_dropout
        # assert self.head_dim * num_heads == self.embed_dim, "embed_dim must be divisible by num_heads"
        self.head_dim = embed_dim // num_heads
        self.bagging_head_dim = int((self.head_dim*self.qk_big/self.bagging_num))
        self.scaling = self.bagging_head_dim ** -0.5
        self.ffn_dim = args.encoder_ffn_embed_dim
        self.fc1 = linear_bm(args, embed_dim, (2*self.qk_big+1)*embed_dim+self.ffn_dim, cur_fc='in')
        self.fc2 = linear_bm(args, embed_dim+self.ffn_dim, embed_dim, cur_fc='out')
        self.activation_fn = utils.get_activation_fn(
            activation=getattr(args, 'activation_fn', 'relu')
        )
    ...
    def forward(self, query, key, value, key_padding_mask=None):
        ...
        embed_dim = self.embed_dim
        ins = self.fc1(query)
        q = torch.narrow(ins, -1, 0, self.qk_big*embed_dim)
        k = torch.narrow(ins, -1,  self.qk_big*embed_dim, self.qk_big*embed_dim)
        v = torch.narrow(ins, -1, 2*self.qk_big*embed_dim, embed_dim)
        ffn_out = F.dropout(self.activation_fn(torch.narrow(ins, -1, (2*self.qk_big+1)*embed_dim, self.ffn_dim)), p=self.activation_dropout, training=self.training)
        q = q * self.scaling
        # print('self.head_dim*self.qk_big/self.bagging_num',self.head_dim*self.qk_big/self.bagging_num)
        q = q.contiguous().view(tgt_len, bsz * self.num_heads*self.bagging_num, self.bagging_head_dim).transpose(0, 1)
        k = k.contiguous().view(-1, bsz * self.num_heads*self.bagging_num, self.bagging_head_dim).transpose(0, 1)
        v = v.contiguous().view(-1, bsz * self.num_heads, self.head_dim).transpose(0, 1)
        src_len = k.size(1)
        # This is part of a workaround to get around fork/join parallelism
        # not supporting Optional types.
        ...
        if key_padding_mask is not None:
            assert key_padding_mask.size(0) == bsz
            assert key_padding_mask.size(1) == src_len
        if self.qk_Dropout is not None:
            q = self.qk_Dropout(q)
            k = self.qk_Dropout(k)
        attn_weights = torch.bmm(q, k.transpose(1, 2))  # [bsz * self.num_heads*self.bagging_num, tgt_len, src_len]
        if self.bagging_num > 1:
            attn_weights = attn_weights.reshape((bsz*self.num_heads,self.bagging_num,tgt_len,src_len))
            if self.qk_bagging == 1:
                attn_weights,_ = torch.max(attn_weights,dim=1)
            elif self.qk_bagging == 2:
...
class TransformerBMDecoderLayerBagging(nn.Module):
    """Decoder layer block.
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False):
        ...
        self.bagging_num = args.bagging_num if 'bagging_num' in args else 1
        self.qk_dropout = args.qk_dropout if 'qk_dropout' in args else 0
        self.self_attn = MultiheadAttentionDecselfBagging(
            embed_dim=self.embed_dim,
            num_heads=args.decoder_attention_heads,
            args=args,
            dropout=args.attention_dropout,
        ...
        if no_encoder_attn:
            ...
            self.encoder_attn = MultiheadAttentionContextBagging(
                self.embed_dim, args.decoder_attention_heads,args=args,
                dropout=args.attention_dropout,
            )
        self.ffn_dim = args.encoder_ffn_embed_dim
        ...
        self.bm_fc4 = args.bm_fc4 if 'bm_fc4' in args else 0
        if self.bm_fc3:
            self.fc1 = linear_bm(args, self.embed_dim, (3*self.qk_big) * self.embed_dim + self.ffn_dim, cur_fc='in')
            self.fc3 = linear_bm(args, self.embed_dim, self.qk_big*self.embed_dim,cur_fc='in')
        else:
            self.fc1 = linear_bm(args, self.embed_dim, (3*self.qk_big+1)*self.embed_dim+self.ffn_dim, cur_fc='in')
        if self.bm_fc4:
            self.fc2 = linear_bm(args, self.embed_dim + self.ffn_dim, self.embed_dim, cur_fc='out')
            self.fc4 = linear_bm(args, self.embed_dim, self.embed_dim, cur_fc='out')
        else:
    ...
    def forward(
        ...
        x = self.maybe_layer_norm(self.layer_norm, x, before=True)
        ins = self.fc1(x)
        q1 = torch.narrow(ins, -1, 0, self.qk_big*self.embed_dim)
        k1 = torch.narrow(ins, -1, self.qk_big*self.embed_dim, self.qk_big*self.embed_dim)
        v1 = torch.narrow(ins, -1, 2*self.qk_big*self.embed_dim, self.embed_dim)
        if self.bm_fc3:
            q2 = self.fc3(x)
            ffn_in = torch.narrow(ins, -1, (2*self.qk_big+1)*self.embed_dim, self.ffn_dim)
        else:
            q2 = torch.narrow(ins, -1, (2*self.qk_big+1)*self.embed_dim, self.qk_big*self.embed_dim)
            ffn_in = torch.narrow(ins, -1, (3*self.qk_big+1)*self.embed_dim, self.ffn_dim)
        if prev_self_attn_state is not None:
            if incremental_state is None:
                incremental_state = {}
            prev_key, prev_value = prev_self_attn_state
...
class MultiheadAttentionDecselfBagging(nn.Module):
    """Multi-headed attention.
    ...
    def forward(self, q, k, v, key_padding_mask=None, incremental_state=None,
        ...
        if saved_state is not None:
            # saved states are stored with shape (bsz, num_heads, seq_len, head_dim)
            if 'prev_key' in saved_state:
                prev_key = saved_state['prev_key'].view(bsz * self.num_heads*self.bagging_num, -1, self.bagging_head_dim)
                if static_kv:
                    k = prev_key
                else:
                    k = torch.cat((prev_key, k), dim=1)
            if 'prev_value' in saved_state:
                ...
                if static_kv:
                    ...
                    v = torch.cat((prev_value, v), dim=1)
            saved_state['prev_key'] = k.view(bsz, self.num_heads*self.bagging_num, -1, self.bagging_head_dim)
            saved_state['prev_value'] = v.view(bsz, self.num_heads, -1, self.head_dim)
...
class MultiheadAttentionContextBagging(nn.Module):
    """Multi-headed attention.
    ...
    def __init__(self, embed_dim, num_heads, args, kdim=None, vdim=None, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False):
        ...
        # else:
        #     self.bias_k = self.bias_v = None
        self.kv_fc = linear_bm(args,embed_dim, (self.qk_big+1)*embed_dim,cur_fc='in')
        # self.add_zero_attn = add_zero_attn
        self.onnx_trace = False
    ...
    def forward(self, q, key, value, key_padding_mask=None, incremental_state=None,
        ...
        if key is None:
            ...
            kv = self.kv_fc(key)
            k = torch.narrow(kv, -1, 0, self.qk_big*self.embed_dim)
            v = torch.narrow(kv, -1, self.qk_big*self.embed_dim, self.embed_dim)
        q *= self.scaling
        q = q.contiguous().view(tgt_len, bsz * self.num_heads*self.bagging_num, self.bagging_head_dim).transpose(0, 1)
        if k is not None:
            k = k.contiguous().view(-1, bsz * self.num_heads*self.bagging_num, self.bagging_head_dim).transpose(0, 1)
